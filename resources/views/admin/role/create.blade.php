    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header bg-light">
                <h4 class="modal-title" id="myExtraLargeModal">Add {{ $title }}</h4><button
            ...
            <div class="modal-body dark-modal">
                <form class="add-new-user pt-0" id="add-user">
                                                        ...
                                                        @foreach ($pc as $permission)
                                                            <div class="form-check me-3 me-lg-5">
                                                                <label class="form-check-label"
                                                                    for="p-{{ $permission->id }}">
                                                                    <input
                                                                        class="form-check-input permission {{ $permission->category }}"
                                                                        type="checkbox" id="p-{{ $permission->id }}"
                                                                        name="{{ $permission->id }}" />
                    ...
                    <button type="button"
                        onclick="addFormData(event,'post','{{ $url }}','{{ $url }}','add-user')"
